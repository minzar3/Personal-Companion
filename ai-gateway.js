...
function buildClassifyPrompt(rawText) {
  const systemPrompt =
    'تو یک دستیار دسته‌بندی متن هستی داخل اپلیکیشن «Personal Companion». ' +
    'کاربر یک متن خام و بدون‌ساختار نوشته. باید تشخیص بدی این متن به کدوم دسته نزدیک‌تره: ' +
    '«Project» (یک پروژه یا هدف بزرگ با چند مرحله)، «Task» (یک کار مشخص، کوچک و قابل انجام که فقط باید انجامش داد)، ' +
    '«Problem» (چیزی که هنوز روشن نیست — کاربر نمی‌داند چطور شروع کند، چه تصمیمی بگیرد، یا دچار یک سردرگمی/نگرانی حل‌نشده است؛ ' +
    'نشانه‌اش عبارت‌هایی مثل «نمی‌دانم چطور...»، «نمی‌دانم باید...»، یا توصیف یک وضعیت نامشخص است، نه یک اقدام مشخص)، ' +
    'یا «Note» (یک نکته، ایده یا اطلاعات صرفاً برای نگهداری، بدون نیاز فوری به فهم یا اقدام). ' +
    'فقط و فقط یک JSON خام و معتبر برگردون، دقیقاً با این شکل، بدون هیچ متن اضافه یا Markdown fence: ' +
    '{"suggested_type": "Project یا Task یا Problem یا Note", "reason": "یک جملهٔ کوتاه فارسی که دلیل انتخابت رو توضیح بده"}';
